...
const SECTION_KEYWORDS = [
  ...
  "skit", "spoken", "part", "section", "guitar solo", "piano solo",
  // Spanish / Portuguese
  "verso", "coro", "estribillo", "puente", "ponte", "refrão", "refrao", "introducción", "introducao", "estanza", "letra", "estrofa",
  // French / German / Italian
  "couplet", "refrain", "strophe", "brücke", "brucke", "ritornello", "strofa", "vorspiel", "nachspiel", "introduzione",
  // Turkish / Russian
  "nakarat", "köprü", "kopru", "giriş", "çıkış", "куплет", "припев", "бридж", "интро", "аутро", "вступление", "концовка",
  // Hindi / Punjabi
...
export function cleanLyrics(raw: string): { lines: string[]; tagsRemoved: string[] } {
  ...
  for (const rawLine of withoutTags.split(/\r?\n/)) {
    ...
    const lower = line.toLowerCase().replace(/[:\-–—]+$/, "").replace(/\s*\(?x?\s*\d+\s*\)?$/i, "").trim();
    const isStandaloneHeader = SECTION_KEYWORDS.some((kw) => lower === kw || lower.startsWith(kw + " ") || lower.endsWith(" " + kw)) ||
      /^(intro|verse|verso|couplet|strophe|куплет|chorus|coro|refrain|припев|サビ|副歌|主歌|bridge|ponte|puente|бридж|hook|outro|interlude|solo|part|section)\s*[:\-–—]?\s*\d*[a-c]?$/i.test(lower);

    if (isStandaloneHeader) {
      tagsRemoved.push(line);
      continue;
    }
